...
use App\Http\Requests\Api\AddRecordPlanYearsRequest;
use App\Http\Requests\Api\IdStudentRequest;
use App\Http\Requests\Api\UpdateRecordPlanYearsRequest;
use Carbon\Carbon;
...
private function checkYearPlan($studentId)
{
    ...
    */
    ...
    public function IsCompleated(IdRequest $request)
{
   ...
   public function add_record_plan_years(AddRecordPlanYearsRequest $request)
{
    DB::beginTransaction();

    try {

        // البحث عن الخطة الحالية للطالب
        $oldPlan = RecordPlanYearsModel::where(
            'student_id',
            $request->student_id
        )->first();
        ...
        if ($oldPlan) {
            ...
            // تاريخ انتهاء الخطة = بعد سنة من start_date
            $expirationDate = \Carbon\Carbon::parse($oldPlan->start_date)
                ->addYear();

            // إذا كانت الخطة ما زالت فعالة
            if (now()->lt($expirationDate)) {

                DB::rollBack();

                return response()->json([
                    'message' => 'هذا الطالب لديه خطة سنوية فعالة بالفعل.',
                    'record_plan_years' => $oldPlan,
                    'expires_at' => $expirationDate->toDateString(),
                ], 409);
            }

            // إذا انتهت الخطة القديمة
            $oldPlan->delete();
        }
    ...
    public function get_special_record_plan_years(IdStudentRequest $request)
    {
        $record = RecordPlanYearsModel::where('student_id', $request->student_id)
            ->latest('id')
            ->first();
    ...
    public function update_record_plan_years(UpdateRecordPlanYearsRequest $request)
    {
        DB::beginTransaction();
    ...
    public function delete_record_plan_years(IdRequest $request)
    {
        DB::beginTransaction();
