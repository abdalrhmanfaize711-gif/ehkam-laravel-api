...
use App\Http\Requests\Api\AddHalaqaRequest;
use App\Http\Requests\Api\IdRequest;
use App\Http\Requests\Api\UpdateHalaqaRequest;
use App\Http\Requests\Api\DeleteHalaqaRequest;
...
{
   public function add_halaqat(AddHalaqaRequest $request)
{
    ...
    function get_all_halaqat(Request $request){
         ...
         return  response()->json(['message' => 'تم استرجاع جميع الحلقات بنجاح', 'halaqat' => $halaqat], 201);
        }
    ...
    public function delete_halaqa(DeleteHalaqaRequest $request)
{
    $request->validate([
    ...
    function get_special_halaqa(Request $request){
         $halaqa = HalaqatModel::findOrFail($request->id);
         return  response()->json(['message' => 'تم استرجاع بيانات الحلقة بنجاح', 'halaqa' => $halaqa], 201);
    }
